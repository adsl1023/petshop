...
<p align="center"><font size="5" color="#009999">寵物訂單記錄查詢(給管理者)</font></p><br> 
<div class="container">
 <div class="top">
    </div>
  <form id="form1" name="form1" method="post" action="" class="form-horizontal"enctype="multipart/form-data" >
     ...
     <div class="form-group">
            <label for="預約紀錄" class="col-sm-4 control-label">預約記錄：</label>
           
      <table width="396" height="261" border="1" align="center">
  <tr>
    <th width="77" scope="col">訂單編號</th>
    <th width="81" scope="col">訂購人帳號</th>
    <th width="111" scope="col">商品名稱</th>
    <th width="99" scope="col">數量</th>
    <th width="99" scope="col">商品價格</th>  
     <th width="99" scope="col">合計</th>
     <th width="99" scope="col">出貨狀況</th>
      <th width="99" scope="col">取貨日期</th>
  </tr>
<?php 
							$sql="select * from 訂單";
							$row=mysql_query($sql);
...
background-color:rgb(100,200,50);
  ...
  <tr>
    <th scope="row"><?php echo mysql_result($row,$i,"訂單編號");?></th>
    <td><?php echo mysql_result($row,$i,"訂購人帳號");?></td>
    <td><?php echo mysql_result($row,$i,"商品名稱");?></td>
    <td><?php echo mysql_result($row,$i,"商品數量");?></td>
     <td><?php echo mysql_result($row,$i,"商品價格");?></td>
     <td><?php echo mysql_result($row,$i,"合計");?></td>
     <td><?php echo mysql_result($row,$i,"出貨狀況");?></td>
     <td><?php echo mysql_result($row,$i,"取貨日期");?></td>
  </tr>
 <?php }?>
</table>
 <style>
...
echo'<a href="searchorderforadmin.php?page='.$i.'">[' . $i . ']</a>';    //顯示本頁的分頁數字會以[ ]包起來
?>     
<?php 
}
else if($i == $count) {
echo '<a href="searchorderforadmin.php?page='.$i.'" >' . $i . '</a>';      //顯示末頁的分頁數字
?>
<?php
}
else {
echo '<a href="searchorderforadmin.php?page='.$i.'">' . $i . ',</a>';    //顯示其他的分頁數字會以" , "作區隔
} 
}
?> 
      </div>
