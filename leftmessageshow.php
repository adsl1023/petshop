			...
			<div id="contents">
				<div class="box">
					...
					<div class="body">
							...
							<?php 
							$sql="select * from message where 商品代號='".$_GET['shopname']."'order by 留言時間 desc";
							echo $sql;
							$row=mysql_query($sql);
...
//-->                 
							...
							for ($i=$min; $i<$max ; $i++) {
								...
								<div class="small1">
									<div class="right1">
										<h2><?php echo mysql_result($row,$i,"商品代號");?></h2>
										<?php echo mysql_result($row,$i,"留言主旨");?>
                                        <h2><?php echo mysql_result($row,$i,"留言內容");?></h2>
                                        </div>
									<div  class="left1">
										<h2><?php echo mysql_result($row,$i,"留言時間");?></h2>
								                            	</div>
...
for($i=1;$i<=$count;$i++) {
if($i == $counts) {
echo'<a class="linkcss" href="leftmessageshow.php?shopname='.$_GET['shopname'].'&page='.$i.'" >[' . $i . ']</a>';    //顯示本頁的分頁數字會以[ ]包起來
}            
else if($i == $count) {
echo '<a class="linkcss" href="leftmessageshow.php?shopname='.$_GET['shopname'].'&page='.$i.'" >' . $i . '</a>';      //顯示末頁的分頁數字
}
else {
echo '<a  class="linkcss" href="leftmessageshow.php?shopname='.$_GET['shopname'].'&page='.$i.'">' . $i . ',</a>';    //顯示其他的分頁數字會以" , "作區隔
}        
}  
?> 
</div>
