...
.button1:active {
   ...
   background: #1b435e;
							...
							<h1>線上Q&A 
     <a href="addpost.php" class="button1">我要發文</a></h1>
                            
							<ul id="foods">
                            <?PHP for ($i=$min; $i<$max ; $i++) {
								 ?>
								<li>
									<h2><a href="food.php?topic=<?php echo mysql_result($row,$i,"識別碼");?>"><?php echo mysql_result($row,$i,"主題");?></a></h2>
									<div class="infos">
										<a href="foods.php"><img src="images/product<?php echo mysql_result($row,$i,"圖檔");?>." alt="Img" height="169" width="780"><span class="cover"></span></a>
										<p>
											<span><?php echo mysql_result($row,$i,"主題");?></span> <?php echo mysql_result($row,$i,"簡單描述");?>
           <?php echo mysql_result($row,$i,"發文帳號");?>         
 <?php echo mysql_result($row,$i,"最後上傳日期");?>
										</p>
									</div>
								</li>
                            <?php }   ?>
...
for($i=1;$i<=$count;$i++) {
if($i == $counts) {
echo'<a href="foods.php?page='.$i.'" >[' . $i . ']</a>';    //顯示本頁的分頁數字會以[ ]包起來
}            
else if($i == $count) {
echo '<a href="foods.php?page='.$i.'" >' . $i . '</a>';      //顯示末頁的分頁數字
}
else {
echo '<a href="foods.php?page='.$i.'">' . $i . ',</a>';    //顯示其他的分頁數字會以" , "作區隔
}        
}  
?>
		 					</ul>
