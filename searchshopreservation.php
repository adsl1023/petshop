...
<body>
<p align="center"><font size="5" color="#009999">寵物商品記錄查詢(給顧客)</font></p><br> 
<div class="container">
 <div class="top">
    </div>
  <form id="form1" name="form1" method="post" action="" class="form-horizontal"enctype="multipart/form-data" >
     <div class="form-group">
         ...
         <div>
				...
				$sql="select * from 訂單 where 訂購人帳號='".$_SESSION['帳號']."'";
							$row=mysql_query($sql);
...
//-->                 
 ...
 <?php }else{ echo $_SESSION['帳號']; ?> <input type="hidden" class="form-control" placeholder="自動填入" name="guestNumber" id="guestNumber" value="<?php echo $$_SESSION['帳號']; ?>" />
  ...
  <tr>
    ...
    <th width="111" scope="col">商品數量</th>
...
echo'<a href="searchshopreservation.php?page='.$i.'" >[' . $i . ']</a>';    //顯示本頁的分頁數字會以[ ]包起來
?>     
<?php 
}
else if($i == $count) {
echo '<a href="searchshopreservation.php?page='.$i.'" >' . $i . '</a>';      //顯示末頁的分頁數字
?>
<?php
}
else {
echo '<a href="searchshopreservation.php?page='.$i.'">' . $i . ',</a>';    //顯示其他的分頁數字會以" , "作區隔
} 
}
}?> 
      </div>
