...
<p align="center"><font size="5" color="#009999">寵物預約記錄查詢(給管理者)</font></p><br> 
<div class="container">
 <div class="top">
    </div>
  ...
  <tr>
    ...
    <th width="99" scope="col">預約表單送出時間</th>   
  </tr>
<?php 
							$sql="select * from designreservation";
							$row=mysql_query($sql);
  ...
  <tr>
    ...
    <td><?php echo mysql_result($row,$i,"預約名額");?></td>
     <td><?php echo mysql_result($row,$i,"預約表單送出時間");?></td>
  </tr>
 <?php }?>
</table>
 <style>
...
echo'<a href="searchreservationforadmin.php?page='.$i.'" >[' . $i . ']</a>';    //顯示本頁的分頁數字會以[ ]包起來
?>     
<?php 
}
else if($i == $count) {
echo '<a href="searchreservationforadmin.php?page='.$i.'" >' . $i . '</a>';      //顯示末頁的分頁數字
?>
<?php
}
else {
echo '<a href="searchreservationforadmin.php?page='.$i.'">' . $i . ',</a>';    //顯示其他的分頁數字會以" , "作區隔
} 
}
?> 
      </div>
