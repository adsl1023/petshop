...
echo $file;
echo move_uploaded_file($_FILES['file']['tmp_name'],"images/designer/".$file);   // 搬移上傳檔案
...
$sql_select="select max(識別碼) from designer";
$result=mysql_query($sql_select);
$row=mysql_fetch_row($result);
$start=$row[0];
$end=$start+1;
echo $end;
$sql_insert = "insert into designer (識別碼,預約時間,設計師名稱,設計師自我介紹,設計師圖檔,設計師名額) values ('".$end."','". $designtime . "','" . $design . "','" . $customidea. "','" .$file. "','".$reservationamount."')";
echo $sql_insert;
